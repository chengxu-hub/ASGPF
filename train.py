...
def train(model, dataloaders, args, device, save_dir, log, tbx):
    ...
    # class_weights = torch.tensor([34.6])
    # 两个类别的样本数量
    sz_sample_count = 6257  # 第一个类别的样本数量
    nosz_sample_count = 218144  # 第二个类别的样本数量

    # 计算每个类别的样本权重，即样本数量的倒数
    class_weights = nosz_sample_count / sz_sample_count
    class_weights = torch.tensor([class_weights]).to(device, dtype=torch.float32)

    if args.task == 'detection':
        # 对于二分类任务，使用BCEWithLogitsLoss，并传递类别权重
...
def evaluate(
    ...
    # 示例：根据样本数量分配权重
    num_positive_samples = sum(y_true_all)  # 正类别样本数量
    num_negative_samples = len(y_true_all) - num_positive_samples  # 负类别样本数量
    weight_positive = 1.0 / num_positive_samples
    weight_negative = 1.0 / num_negative_samples
    sample_weights = [weight_positive if label == 1 else weight_negative for label in y_true_all]
    ...
    results_list = [('loss', eval_loss),
                    ('acc', scores_dict['acc']),
                    ('F1', scores_dict['F1']),
                    ('weighted_F1', scores_dict['weighted_F1']),
...
#
#
#         # 初始化两个空列表来存储train_loader和dev_loader的y
#         train_labels = []
#         dev_labels = []
#
#         # 迭代train_loader并获取train_loader中的y
#         for x, y, seq_lengths, supports, _, _ in train_loader:
#             train_labels.extend(y.tolist())  # 将train_loader中的y添加到train_labels
#             del x, y, seq_lengths, supports
#             torch.cuda.empty_cache()  # 释放GPU内存
#             gc.collect()  # 执行Python内存垃圾回收
#
#         # 迭代dev_loader并获取dev_loader中的y
#         for x, y, seq_lengths, supports, _, _ in dev_loader:
#             dev_labels.extend(y.tolist())  # 将dev_loader中的y添加到dev_labels
#             del x, y, seq_lengths, supports
#             torch.cuda.empty_cache()  # 释放GPU内存
#             gc.collect()  # 执行Python内存垃圾回收
#
#         # 合并train_labels和dev_labels
...
#                 x, y, seq_lengths, supports, adj_mat, writeout_fn = self.data[idx]
#                 supports = [torch.FloatTensor(np.array(s)) for s in supports]  # 将supports中的每个元素转换为张量
#                 return x, y, seq_lengths, supports, adj_mat, writeout_fn
#
#
#         # 创建 K 折交叉验证对象
#         kf = StratifiedKFold(n_splits=K, shuffle=True)
#
#         for i, (strat_train_indices, strat_dev_indices) in enumerate(kf.split(data, all_labels)):
#             log.info('Training for fold {}/{}'.format(i + 1, K))
#             train_indices = strat_train_indices
#             dev_indices = strat_dev_indices
#
#             # 获取训练数据和验证数据
...
#                 dataset = CustomDataset(split_data[split])
#
#                 if split == 'train':
#                     shuffle = True
...
#
#                 new_loader = DataLoader(dataset=dataset, shuffle=shuffle, batch_size=batch_size,
#                                         num_workers=0)
#                 new_dataloaders[split] = new_loader
#
#
#             # 训练模型使用 train_data
#             # Train
#             train(model, new_dataloaders, args, device, args.save_dir, log, tbx)
#
#             # Load best model after training finished
#             best_path = os.path.join(args.save_dir, 'best.pth.tar')
#             model = utils.load_model_checkpoint(best_path, model)
...
#             log.info('Training DONE. Evaluating model...')
#             # 验证模型使用 val_data
#             log.info('Validating for fold {}/{}'.format(i + 1, K))
#             dev_results = evaluate(model,
#                                    new_dataloaders['dev'],
#                                    args,
#                                    args.save_dir,
...
#             dev_results_str = ', '.join('{}: {:.3f}'.format(k, v)
#                                         for k, v in dev_results.items())
#             log.info('DEV set prediction results for flod {}/{}: {}'.format(i + 1, K, dev_results_str))
#             # 在折结束时释放内存
#             del train_data, dev_data, split_data, new_dataloaders
#             torch.cuda.empty_cache()  # 释放GPU内存
#             gc.collect()  # 执行Python内存垃圾回收
#
#
...
#
#
# def train(model, new_dataloaders, args, device, save_dir, log, tbx):
#     """
#     Perform training and evaluate on val set
#     """
#
...
#     # class_weights = torch.tensor([34.6])
#     # 两个类别的样本数量
#     sz_sample_count = 13851  # 第一个类别的样本数量
#     nosz_sample_count = 210551  # 第二个类别的样本数量
#
#     # 计算每个类别的样本权重，即样本数量的倒数
#     class_weights = nosz_sample_count / sz_sample_count
#     class_weights = torch.tensor([class_weights]).to(device, dtype=torch.float32)
...
#
#     # Data loaders
#     train_loader = new_dataloaders['train']
#     dev_loader = new_dataloaders['dev']
#
#
#     # Get saver
#     saver = utils.CheckpointSaver(save_dir,
